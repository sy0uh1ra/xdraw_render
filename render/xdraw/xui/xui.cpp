...
namespace xui {
	...
	namespace binds {
		...
		void process( const input_state& input )
		{
			...
			for ( auto* s : reg.settings )
			{
				...
				{
					const auto was_active = s->bind.active;
					s->bind.active = held;
					s->value = held;

					if ( s->bind.active && !was_active && s->bind.excludes )
					{
						s->bind.excludes->bind.active = false;
						s->bind.excludes->value = false;
					}
	...
	bool begin_window( std::string_view title, float& x, float& y, float& w, float& h, bool resizable, float min_w, float min_h )
	{
		...
		const auto grip_hovered = resizable && input.in_rect( grip );
		const auto window_hovered = input.in_rect( abs );
		...
		if ( c.active_window == id && input.mouse_down&& c.active_slider == null_id&& c.active_resize == null_id&& c.active_text_input == null_id&& c.active_child_scroll == null_id )
		{
		...
		dl.rect_filled( abs.x, abs.y, abs.w, abs.h, s.window_bg, xdraw::corner_radius{ r } );

		if ( s.border_thickness > 0.0f )
		{
			dl.rect( abs.x, abs.y, abs.w, abs.h, s.window_border, xdraw::corner_radius{ r }, s.border_thickness );
		}
	...
	void end_child( )
	{
		...
		if ( win && win->group_id != null_id )
		{
			...
			if ( win->scrollable )
			{
				...
				const auto inset = 0.0f;
				const auto full_w = win->bounds.w;
				const auto full_right = win->bounds.right( );
	...
	bool begin_popup( std::string_view label, float width )
	{
		...
		if ( ov && !ov->is_closed( ) )
		{
			...
			const auto animated_h = popup_rect.h;
			...
			if ( !ov->is_closing( ) )
			{
				top_dl.rect_filled_blurred( popup_rect.x, popup_rect.y, popup_rect.w, animated_h, xdraw::corner_radius{ s.popup_rounding } );
			}
			...
			const auto fade = [&]( xdraw::color col ) -> xdraw::color {
				col.a = static_cast< std::uint8_t >( col.a * alpha_mult );
				return col;
			};

			push_style_color( style_col::text,                       fade( s.text ) );
			push_style_color( style_col::text_dim,                   fade( s.text_dim ) );
			push_style_color( style_col::checkbox_bg,                fade( s.checkbox_bg ) );
			push_style_color( style_col::checkbox_border,            fade( s.checkbox_border ) );
			push_style_color( style_col::checkbox_mark,              fade( s.checkbox_mark ) );
			push_style_color( style_col::checkbox_mark_icon,         fade( s.checkbox_mark_icon ) );
			push_style_color( style_col::slider_track,               fade( s.slider_track ) );
			push_style_color( style_col::slider_fill,                fade( s.slider_fill ) );
			push_style_color( style_col::combo_bg,                   fade( s.combo_bg ) );
			push_style_color( style_col::combo_border,               fade( s.combo_border ) );
			push_style_color( style_col::combo_arrow,                fade( s.combo_arrow ) );
			push_style_color( style_col::combo_hovered,              fade( s.combo_hovered ) );
			push_style_color( style_col::button_bg,                  fade( s.button_bg ) );
			push_style_color( style_col::button_border,              fade( s.button_border ) );
			push_style_color( style_col::button_hovered,             fade( s.button_hovered ) );
			push_style_color( style_col::button_active,              fade( s.button_active ) );
			push_style_color( style_col::text_input_bg,              fade( s.text_input_bg ) );
			push_style_color( style_col::text_input_border,          fade( s.text_input_border ) );
			push_style_color( style_col::separator,                  fade( s.separator ) );
			push_style_color( style_col::accent,                     fade( s.accent ) );
	...
	bool checkbox( std::string_view label, setting& s )
	{
		...
		const auto hovered = can_interact && input.in_rect( extended );
		if ( hovered && input.mouse_clicked )
		{
			s.value = !s.value;
			s.bind.active = s.value;
		...
		const auto r = st.checkbox_rounding;
		...
		const auto bg = lighten( st.checkbox_bg, 1.0f + hover_anim * 0.4f );
		dl.rect_filled( abs.x, abs.y, abs.w, abs.h, bg, xdraw::corner_radius{ r } );

		if ( ease_t > 0.01f )
		{
			const auto fill = xdraw::color{ st.checkbox_mark.r, st.checkbox_mark.g, st.checkbox_mark.b, static_cast< std::uint8_t >( st.checkbox_mark.a * ease_t ) };
			dl.rect_filled( abs.x, abs.y, abs.w, abs.h, fill, xdraw::corner_radius{ r } );

			const auto mark_a = static_cast< std::uint8_t >( 255 * ease_t );
			const auto mark_col = xdraw::color{ st.checkbox_mark_icon.r, st.checkbox_mark_icon.g, st.checkbox_mark_icon.b, mark_a };
			const auto cx = abs.x + abs.w * 0.5f;
			const auto cy = abs.y + abs.h * 0.5f;
			const auto scale = 0.6f + ease_t * 0.4f;
			const auto thickness = std::max( 1.2f, abs.w * 0.1f );

			const std::array<float, 6> pts
			{
				cx + ( -abs.w * 0.28f ) * scale, cy + ( abs.h * 0.0f ) * scale,
				cx + ( -abs.w * 0.08f ) * scale, cy + ( abs.h * 0.18f ) * scale,
				cx + ( abs.w * 0.28f ) * scale, cy + ( -abs.h * 0.22f ) * scale
			};

			dl.polyline( pts, mark_col, false, thickness );
		}
	...
	namespace {
		...
		inline bool slider( std::uintptr_t id, std::string_view label, T& v, T v_min, T v_max, std::string_view fmt )
		{
			...
			const auto total_height = text_height + spacing + track_height;

			const auto abs = layout::item( slider_width, total_height, text_height );
			const auto track_y = abs.y + text_height + spacing;
			...
			{
				const auto hovered = can_interact && input.in_rect( track_rect );
				...
				const auto value_col = lerp( s.text_dim, s.text, anim::get( id + 1000000 ) );
			...
			const auto hovered_or_active = ( can_interact && input.in_rect( track_rect ) ) || c.active_slider == id;
			...
			const auto fill_w = slider_width * norm_pos;
			const auto track_r = track_rect.h * 0.5f - 0.5f;
			...
			dl.rect_filled( track_rect.x, track_rect.y, track_rect.w, track_rect.h, s.slider_track, xdraw::corner_radius{ track_r } );

			if ( fill_w > 0.5f )
			{
				const auto fill_left = lighten( s.slider_fill, 1.1f + hover_anim_val * 0.1f );
				const auto fill_right = darken( s.slider_fill, 0.85f );
				dl.rect_filled_gradient( track_rect.x, track_rect.y, fill_w, track_rect.h, fill_left, fill_right, fill_right, fill_left, xdraw::corner_radius{ track_r } );
			}
	...
	namespace {
		...
		public:
			...
			void render( const style& style, const input_state& input ) override
			{
				...
				if ( !this->m_closing )
				{
					dl.rect_filled_blurred( dd.x, dd.y, dd.w, animated_h, xdraw::corner_radius{ pr } );
				}
	...
	namespace {
		...
		public:
			...
			void render( const style& style, const input_state& input ) override
			{
				...
				const auto pr = style.combo_popup_rounding;
				const auto cr = style.checkbox_rounding;
				...
				for ( int i = 0; i < this->m_count; ++i )
				{
					...
					{
						auto sel = style.combo_popup_item_selected;
						sel.a = static_cast< std::uint8_t >( sel.a * item_alpha * ce );
						dl.rect_filled( ir.x, ir.y + slide, ir.w, ir.h, sel, xdraw::corner_radius{ ir_tl, ir_tr, ir_br, ir_bl } );
					}

					auto text_col = style.text_dim;
					text_col = lerp( text_col, style.text, std::max( he, ce ) );
					text_col.a = static_cast< std::uint8_t >( text_col.a * item_alpha );

					const auto [tw, th] = xdraw::measure_text( this->m_items[ i ] );
					dl.text( ir.x + style.frame_pad_x, ir.y + ( this->m_item_h - th ) * 0.5f + slide, this->m_items[ i ], text_col );
				}
	...
	namespace {
		...
		public:
			...
			void render( const style& style, const input_state& ) override
			{
				...
				if ( !this->m_closing )
				{
					dl.rect_filled_blurred( sx, sy, sw, sh, xdraw::corner_radius{ pr } );
				}
