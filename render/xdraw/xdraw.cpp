...
namespace xdraw {
	...
	bool font::rasterize( char32_t cp )
	{
		...
		if ( FT_Load_Char( face, cp, this->load_flags ) != 0 )
		{
			return false;
		}
	...
	static FT_Int32 quality_to_load_flags( int quality )
	{
		switch ( quality )
		{
		case font_quality::nonantialiased:
			return FT_LOAD_RENDER | FT_LOAD_MONOCHROME | FT_LOAD_TARGET_MONO;

		case font_quality::antialiased:
			return FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL;

		case font_quality::cleartype:
		case font_quality::cleartype_natural:
			return FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL;

		case font_quality::default_quality:
		default:
			return FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL;
		}
	}

	static FT_Int32 default_memory_load_flags( )
	{
		return FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL | FT_LOAD_FORCE_AUTOHINT;
	}

	static void set_font_pixel_size( FT_Face face, float size_px )
	{
		if ( FT_HAS_FIXED_SIZES( face ) )
		{
			...
			{
				const auto height = static_cast< float >( face->available_sizes[ i ].height ) / 64.0f;
				const auto diff = static_cast< int >( std::abs( height - size_px ) );
	...
	static std::string wide_to_utf8( std::wstring_view text )
	{
		...
		const auto len = WideCharToMultiByte( CP_UTF8, 0, text.data( ), static_cast< int >( text.size( ) ), nullptr, 0, nullptr, nullptr );
		if ( len <= 0 )
		{
			return {};
		}
	...
	static bool font_entry_matches( std::wstring_view entry, std::wstring_view family )
	{
		if ( _wcsicmp( entry.data( ), family.data( ) ) == 0 )
		{
			return true;
		}

		if ( entry.size( ) <= family.size( ) )
		{
			return false;
		}

		if ( entry[ family.size( ) ] != L' ' )
		{
			return false;
		}

		return _wcsnicmp( entry.data( ), family.data( ), family.size( ) ) == 0;
	}

	static int score_font_entry( std::wstring_view entry, std::wstring_view family, int weight )
	{
		const auto wants_bold = weight >= 700;
		const auto wants_italic = false;
		const auto is_exact = entry == family;
		const auto is_bold = entry.find( L" Bold" ) != std::wstring_view::npos;
		const auto is_italic = entry.find( L" Italic" ) != std::wstring_view::npos || entry.find( L" Oblique" ) != std::wstring_view::npos;
	...
	static std::wstring resolve_system_font_path( std::wstring_view family, int weight )
	{
		...
		const auto fonts_dir = std::wstring( win_dir ) + L"\\Fonts\\";
		...
		for ( DWORD index = 0;; ++index )
		{
			...
			wchar_t value_data[ MAX_PATH ]{};
			DWORD name_len = static_cast< DWORD >( std::size( value_name ) );
			DWORD data_len = sizeof( value_data );
			DWORD type{};

			const auto err = RegEnumValueW( key, index, value_name, &name_len, nullptr, &type, reinterpret_cast< LPBYTE >( value_data ), &data_len );
			...
			std::wstring_view entry{ value_name };
			const auto suffix = entry.find( L" (" );
			if ( suffix != std::wstring_view::npos )
			{
				entry = entry.substr( 0, suffix );
			}

			if ( !font_entry_matches( entry, family ) )
			{
				continue;
			}

			const auto score = score_font_entry( entry, family, weight );
			if ( score == INT_MIN )
			{
				continue;
			}
			...
			{
				best = candidate_t{ std::wstring( value_data ), score };
			}
		}
		...
		if ( !best )
		{
			const auto fallback = fonts_dir + std::wstring( family ) + L".ttf";
		...
		return fonts_dir + best->file;
	}

	static font* create_font_from_face (FT_Library ft, FT_Face face, float size_px, FT_Int32 load_flags, int atlas_w, int atlas_h) {
		set_font_pixel_size (face, size_px);
		...
		for (auto i = 0x0400; i <= 0x04FF; ++i) {
			f->rasterize (static_cast<char32_t> (i));
		}
	...
	font* load_font( std::span<const std::byte> data, float size_px, int atlas_w, int atlas_h )
	{
		...
		return create_font_from_face( ft, face, size_px, default_memory_load_flags( ), atlas_w, atlas_h );
	}

	font* load_system_font( std::string_view name, float size_px, int weight, int quality, int atlas_w, int atlas_h )
	{
		const auto path = resolve_system_font_path( utf8_to_wide( name ), weight );
		if ( path.empty( ) )
		{
			return nullptr;
		}

		const auto path_utf8 = wide_to_utf8( path );
		if ( path_utf8.empty( ) )
		{
			return nullptr;
		}
		...
		FT_Face face{};
		if ( FT_New_Face( ft, path_utf8.c_str( ), 0, &face ) != 0 )
		{
			FT_Done_FreeType( ft );
			return nullptr;
		}

		if ( weight < 700 && ( face->style_flags & FT_STYLE_FLAG_ITALIC ) )
		{
			FT_Done_Face( face );
			FT_Done_FreeType( ft );
			return nullptr;
		...
		return create_font_from_face( ft, face, size_px, quality_to_load_flags( quality ), atlas_w, atlas_h );
	}
